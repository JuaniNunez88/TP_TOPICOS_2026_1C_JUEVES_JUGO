...
static void confirmar_fijacion(t_estado_juego *eg)
{
    ...
    eg->nivel =
        calcular_nivel(eg->lineas_limpiadas);

    eg->intervalo_caida_ms =
        calcular_intervalo(
            eg->piezas_caidas,
            eg->intervalo_inicial_ms
        );
...
static void validar_rotacion(t_estado_juego *p, int sentido)
{
    ...
    if (!choque_horiz(p, p->pieza_x) &&
        !choque_vert(p, p->pieza_y))
    {
        return;
    }
    ...
    {
        int nueva_x =
            p->pieza_x + mover_lugares[k];

        if (!choque_horiz(p, nueva_x) &&
            !choque_vert(p, p->pieza_y))
        {
            p->pieza_x = nueva_x;
            return;
...
void juego_actualizar(t_estado_juego *eg, int *val)
{
    ...
    case ESTADO_JUGANDO:
    {
        ...
        if (nueva_x != eg->pieza_x &&
            !choque_horiz(eg, nueva_x))
        {
            movio = 1;
            eg->pieza_x = nueva_x;
        }
        ...
        int frames_caida =
            (int)(eg->intervalo_caida_ms / MS_POR_FRAME);

        int frames_fijacion =
            (int)((eg->intervalo_caida_ms * 0.5f)
            / MS_POR_FRAME);

        if (frames_caida < 1)
            frames_caida = 1;

        if (frames_fijacion < 1)
            frames_fijacion = 1;
