#include "fuente.h"
#include "GBT/gbt.h"

static const t_fuente FUENTE_8x8 =
{
    .ancho = 8,
    .alto  = 8,
    .letra = {
        [' ' - 32] = {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},
        ['!' - 32] = {0x18,0x18,0x18,0x18,0x18,0x00,0x18,0x00},
        ['-' - 32] = {0x00,0x00,0x00,0x7E,0x00,0x00,0x00,0x00},
        ['.' - 32] = {0x00,0x00,0x00,0x00,0x00,0x00,0x18,0x00},
        ['/' - 32] = {0x02,0x04,0x08,0x10,0x20,0x40,0x00,0x00},
        ['0' - 32] = {0x3C,0x66,0x6E,0x76,0x66,0x66,0x3C,0x00},
        ['1' - 32] = {0x18,0x38,0x18,0x18,0x18,0x18,0x7E,0x00},
        ['2' - 32] = {0x3C,0x66,0x06,0x0C,0x18,0x30,0x7E,0x00},
        ['3' - 32] = {0x3C,0x66,0x06,0x1C,0x06,0x66,0x3C,0x00},
        ['4' - 32] = {0x0C,0x1C,0x3C,0x6C,0x7E,0x0C,0x0C,0x00},
        ['5' - 32] = {0x7E,0x60,0x7C,0x06,0x06,0x66,0x3C,0x00},
        ['6' - 32] = {0x1C,0x30,0x60,0x7C,0x66,0x66,0x3C,0x00},
        ['7' - 32] = {0x7E,0x06,0x0C,0x18,0x18,0x18,0x18,0x00},
        ['8' - 32] = {0x3C,0x66,0x66,0x3C,0x66,0x66,0x3C,0x00},
        ['9' - 32] = {0x3C,0x66,0x66,0x3E,0x06,0x0C,0x38,0x00},
        [':' - 32] = {0x00,0x18,0x18,0x00,0x18,0x18,0x00,0x00},
        ['A' - 32] = {0x18,0x3C,0x66,0x7E,0x66,0x66,0x66,0x00},
        ['B' - 32] = {0x7C,0x66,0x66,0x7C,0x66,0x66,0x7C,0x00},
        ['C' - 32] = {0x3C,0x66,0x60,0x60,0x60,0x66,0x3C,0x00},
        ['D' - 32] = {0x78,0x6C,0x66,0x66,0x66,0x6C,0x78,0x00},
        ['E' - 32] = {0x7E,0x60,0x60,0x7C,0x60,0x60,0x7E,0x00},
        ['F' - 32] = {0x7E,0x60,0x60,0x7C,0x60,0x60,0x60,0x00},
        ['G' - 32] = {0x3C,0x66,0x60,0x6E,0x66,0x66,0x3C,0x00},
        ['H' - 32] = {0x66,0x66,0x66,0x7E,0x66,0x66,0x66,0x00},
        ['I' - 32] = {0x7E,0x18,0x18,0x18,0x18,0x18,0x7E,0x00},
        ['J' - 32] = {0x06,0x06,0x06,0x06,0x06,0x66,0x3C,0x00},
        ['K' - 32] = {0x66,0x6C,0x78,0x70,0x78,0x6C,0x66,0x00},
        ['L' - 32] = {0x60,0x60,0x60,0x60,0x60,0x60,0x7E,0x00},
        ['M' - 32] = {0x63,0x77,0x7F,0x6B,0x63,0x63,0x63,0x00},
        ['N' - 32] = {0x66,0x76,0x7E,0x7E,0x6E,0x66,0x66,0x00},
        ['O' - 32] = {0x3C,0x66,0x66,0x66,0x66,0x66,0x3C,0x00},
        ['P' - 32] = {0x7C,0x66,0x66,0x7C,0x60,0x60,0x60,0x00},
        ['Q' - 32] = {0x3C,0x66,0x66,0x66,0x6E,0x3C,0x06,0x00},
        ['R' - 32] = {0x7C,0x66,0x66,0x7C,0x6C,0x66,0x66,0x00},
        ['S' - 32] = {0x3C,0x66,0x60,0x3C,0x06,0x66,0x3C,0x00},
        ['T' - 32] = {0x7E,0x18,0x18,0x18,0x18,0x18,0x18,0x00},
        ['U' - 32] = {0x66,0x66,0x66,0x66,0x66,0x66,0x3C,0x00},
        ['V' - 32] = {0x66,0x66,0x66,0x66,0x66,0x3C,0x18,0x00},
        ['W' - 32] = {0x63,0x63,0x63,0x6B,0x7F,0x77,0x63,0x00},
        ['X' - 32] = {0x66,0x66,0x3C,0x18,0x3C,0x66,0x66,0x00},
        ['Y' - 32] = {0x66,0x66,0x3C,0x18,0x18,0x18,0x18,0x00},
        ['Z' - 32] = {0x7E,0x06,0x0C,0x18,0x30,0x60,0x7E,0x00},
    }
};

static const t_fuente FUENTE_8x16 =
{
    .ancho = 8,
    .alto  = 16,
    .letra = {
        [' ' - 32] = {
            0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
            0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00
        },
        ['!' - 32] = {
            0x00,0x18,0x18,0x18,0x18,0x18,0x18,0x18,
            0x18,0x18,0x00,0x00,0x18,0x18,0x00,0x00
        },
        ['-' - 32] = {
            0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xFF,
            0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00
        },
        ['.' - 32] = {
            0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
            0x00,0x00,0x00,0x00,0x18,0x18,0x00,0x00
        },
        ['/' - 32] = {
            0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,
            0x80,0x00,0x00,0x00,0x00,0x00,0x00,0x00
        },
        ['0' - 32] = {
            0x00,0x3C,0x66,0x66,0x6E,0x76,0x66,0x66,
            0x66,0x66,0x66,0x3C,0x00,0x00,0x00,0x00
        },
        ['1' - 32] = {
            0x00,0x18,0x38,0x78,0x18,0x18,0x18,0x18,
            0x18,0x18,0x18,0x7E,0x00,0x00,0x00,0x00
        },
        ['2' - 32] = {
            0x00,0x3C,0x66,0x66,0x06,0x0C,0x18,0x30,
            0x60,0x60,0x60,0x7E,0x00,0x00,0x00,0x00
        },
        ['3' - 32] = {
            0x00,0x3C,0x66,0x06,0x06,0x1C,0x06,0x06,
            0x06,0x06,0x66,0x3C,0x00,0x00,0x00,0x00
        },
        ['4' - 32] = {
            0x00,0x0C,0x1C,0x3C,0x6C,0x6C,0x6C,0x7E,
            0x0C,0x0C,0x0C,0x0C,0x00,0x00,0x00,0x00
        },
        ['5' - 32] = {
            0x00,0x7E,0x60,0x60,0x60,0x7C,0x06,0x06,
            0x06,0x06,0x66,0x3C,0x00,0x00,0x00,0x00
        },
        ['6' - 32] = {
            0x00,0x1C,0x30,0x60,0x60,0x7C,0x66,0x66,
            0x66,0x66,0x66,0x3C,0x00,0x00,0x00,0x00
        },
        ['7' - 32] = {
            0x00,0x7E,0x06,0x06,0x06,0x0C,0x0C,0x18,
            0x18,0x18,0x18,0x18,0x00,0x00,0x00,0x00
        },
        ['8' - 32] = {
            0x00,0x3C,0x66,0x66,0x66,0x3C,0x66,0x66,
            0x66,0x66,0x66,0x3C,0x00,0x00,0x00,0x00
        },
        ['9' - 32] = {
            0x00,0x3C,0x66,0x66,0x66,0x3E,0x06,0x06,
            0x06,0x0C,0x18,0x70,0x00,0x00,0x00,0x00
        },
        [':' - 32] = {
            0x00,0x00,0x00,0x18,0x18,0x00,0x00,0x00,
            0x00,0x18,0x18,0x00,0x00,0x00,0x00,0x00
        },
        ['A' - 32] = {
            0x00,0x18,0x18,0x3C,0x3C,0x66,0x66,0x7E,
            0x66,0x66,0x66,0x66,0x00,0x00,0x00,0x00
        },
        ['B' - 32] = {
            0x00,0x7C,0x66,0x66,0x66,0x7C,0x66,0x66,
            0x66,0x66,0x66,0x7C,0x00,0x00,0x00,0x00
        },
        ['C' - 32] = {
            0x00,0x3C,0x66,0x66,0x60,0x60,0x60,0x60,
            0x60,0x60,0x66,0x3C,0x00,0x00,0x00,0x00
        },
        ['D' - 32] = {
            0x00,0x78,0x6C,0x66,0x66,0x66,0x66,0x66,
            0x66,0x66,0x6C,0x78,0x00,0x00,0x00,0x00
        },
        ['E' - 32] = {
            0x00,0x7E,0x60,0x60,0x60,0x7C,0x60,0x60,
            0x60,0x60,0x60,0x7E,0x00,0x00,0x00,0x00
        },
        ['F' - 32] = {
            0x00,0x7E,0x60,0x60,0x60,0x7C,0x60,0x60,
            0x60,0x60,0x60,0x60,0x00,0x00,0x00,0x00
        },
        ['G' - 32] = {
            0x00,0x3C,0x66,0x60,0x60,0x60,0x6E,0x66,
            0x66,0x66,0x66,0x3C,0x00,0x00,0x00,0x00
        },
        ['H' - 32] = {
            0x00,0x66,0x66,0x66,0x66,0x7E,0x66,0x66,
            0x66,0x66,0x66,0x66,0x00,0x00,0x00,0x00
        },
        ['I' - 32] = {
            0x00,0x7E,0x18,0x18,0x18,0x18,0x18,0x18,
            0x18,0x18,0x18,0x7E,0x00,0x00,0x00,0x00
        },
        ['J' - 32] = {
            0x00,0x06,0x06,0x06,0x06,0x06,0x06,0x06,
            0x06,0x66,0x66,0x3C,0x00,0x00,0x00,0x00
        },
        ['K' - 32] = {
            0x00,0x66,0x66,0x6C,0x6C,0x78,0x78,0x6C,
            0x6C,0x66,0x66,0x66,0x00,0x00,0x00,0x00
        },
        ['L' - 32] = {
            0x00,0x60,0x60,0x60,0x60,0x60,0x60,0x60,
            0x60,0x60,0x60,0x7E,0x00,0x00,0x00,0x00
        },
        ['M' - 32] = {
            0x00,0x63,0x63,0x77,0x77,0x7F,0x6B,0x6B,
            0x63,0x63,0x63,0x63,0x00,0x00,0x00,0x00
        },
        ['N' - 32] = {
            0x00,0x66,0x66,0x76,0x76,0x7E,0x6E,0x6E,
            0x66,0x66,0x66,0x66,0x00,0x00,0x00,0x00
        },
        ['O' - 32] = {
            0x00,0x3C,0x66,0x66,0x66,0x66,0x66,0x66,
            0x66,0x66,0x66,0x3C,0x00,0x00,0x00,0x00
        },
        ['P' - 32] = {
            0x00,0x7C,0x66,0x66,0x66,0x7C,0x60,0x60,
            0x60,0x60,0x60,0x60,0x00,0x00,0x00,0x00
        },
        ['Q' - 32] = {
            0x00,0x3C,0x66,0x66,0x66,0x66,0x66,0x66,
            0x66,0x6E,0x3C,0x06,0x00,0x00,0x00,0x00
        },
        ['R' - 32] = {
            0x00,0x7C,0x66,0x66,0x66,0x7C,0x78,0x6C,
            0x66,0x66,0x66,0x66,0x00,0x00,0x00,0x00
        },
        ['S' - 32] = {
            0x00,0x3C,0x66,0x66,0x60,0x3C,0x06,0x06,
            0x06,0x66,0x66,0x3C,0x00,0x00,0x00,0x00
        },
        ['T' - 32] = {
            0x00,0x7E,0x18,0x18,0x18,0x18,0x18,0x18,
            0x18,0x18,0x18,0x18,0x00,0x00,0x00,0x00
        },
        ['U' - 32] = {
            0x00,0x66,0x66,0x66,0x66,0x66,0x66,0x66,
            0x66,0x66,0x66,0x3C,0x00,0x00,0x00,0x00
        },
        ['V' - 32] = {
            0x00,0x66,0x66,0x66,0x66,0x66,0x66,0x66,
            0x66,0x3C,0x3C,0x18,0x00,0x00,0x00,0x00
        },
        ['W' - 32] = {
            0x00,0x63,0x63,0x63,0x63,0x6B,0x6B,0x7F,
            0x77,0x77,0x63,0x63,0x00,0x00,0x00,0x00
        },
        ['X' - 32] = {
            0x00,0x66,0x66,0x66,0x3C,0x18,0x18,0x18,
            0x3C,0x66,0x66,0x66,0x00,0x00,0x00,0x00
        },
        ['Y' - 32] = {
            0x00,0x66,0x66,0x66,0x3C,0x18,0x18,0x18,
            0x18,0x18,0x18,0x18,0x00,0x00,0x00,0x00
        },
        ['Z' - 32] = {
            0x00,0x7E,0x06,0x06,0x0C,0x18,0x18,0x30,
            0x60,0x60,0x60,0x7E,0x00,0x00,0x00,0x00
        },
    }
};

const t_fuente *fuente_get_8x8()
{
    return &FUENTE_8x8;
}
const t_fuente *fuente_get_8x16()
{
    return &FUENTE_8x16;
}

void fuente_dibujar_char(const t_fuente *f, char c, int x, int y, int color)
{
    if (c < FUENTE_CHAR_MIN || c > FUENTE_CHAR_MAX)
        return;

    int idx = c - FUENTE_CHAR_MIN;
    for (int fila = 0; fila < f->alto; fila++)
    {
        unsigned char byte = f->letra[idx][fila];
        for (int bit = 7; bit >= 0; bit--)
            if (byte & (1 << bit))
                gbt_dibujar_pixel(x + (7 - bit), y + fila, color);
    }
}

void fuente_dibujar_texto(const t_fuente *f, const char *texto, int x, int y, int color)
{
    int cursor_x = x;
    for (int i = 0; texto[i] != '\0'; i++)
    {
        fuente_dibujar_char(f, texto[i], cursor_x, y, color);
        cursor_x += f->ancho + 1;
    }
}
